...
class BundleAddFromListTest(BundleTestBase):
    ...
    def test_add_duplicate(self):
        ...
        expected = escape("Patch '%s' already in bundle" % patch.name)
    ...
    def test_add_new_and_duplicate(self):
        ...
        expected_already = escape("Patch '%s' already in bundle" % patch.name)
        expected_added = escape(
            "Patch '%s' added to bundle" % self.patches[1].name)
