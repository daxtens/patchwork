...
class MailParsingTest(TestCase):
    ...
    def test_invalid_header_char(self):
        """Validate behavior when an invalid character is in a header."""
        mail = read_mail('0012-invalid-header-char.mbox', self.project)
        parse_mail(mail, list_id=self.project.listid)
        self.assertEquals(Patch.objects.all().count(), 1)

    def test_utf8_header_char(self):
        """Validate behaviour when a UTF-8 char is in a header."""
        mail = read_mail('0013-with-utf8-header.mbox')
        parse_mail(mail, list_id=self.project.listid)
        self.assertEquals(Patch.objects.all().count(), 1)
