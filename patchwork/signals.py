...
def patch_change_callback(sender, instance, raw, **kwargs):
    ...
    try:
        orig_patch = Submission.patch_objects.get(pk=instance.pk)
...
def create_patch_completed_event(sender, instance, raw, **kwargs):
    ...
    predecessors = Submission.patch_objects.filter(
    ...
    for successor in Submission.patch_objects.order_by('number').filter(
